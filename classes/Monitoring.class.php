...
class Monitoring {
	...
	public static function domoPlugs() {
		$result = array();
		$commandline = "avahi-browse -tapr | grep -i domo ";
		exec($commandline, $zeroconfList );
    	foreach ($zeroconfList as $service) {
        	$serviceData = explode(';', $service);
        	if ($serviceData[0] == '=' && $serviceData[2] == 'IPv4' && $serviceData[4] == 'Workstation' ) {
            	$result[]  = array(
					'status'	=>	1,
					'plugName' 	=> 	$serviceData[6],
					'plugIP' 	=> 	$serviceData[7]
				);
        	}
    	}
    	return $result;
   	...
   	public static function domoServices() {
    	$result = array();
    	return $result;
	}

   	public static function domoSensors() {
    	$result = array();
    	return $result;
	}
